...
export default function EmpresasPage() {
  ...
  // Função para buscar valor do tipo de serviço "Instalação (por painel)"
  const buscarValorPainelDosTipos = async (empresaId: string) => {
    try {
      const supabase = getSupabaseClient()
      
      // Buscar o tipo de serviço "Instalação (por painel)"
      const { data: tipos } = await supabase
        .from('tipos_servico')
        .select('id')
        .eq('codigo', 'instalacao_painel')
        .eq('ativo', true)
        .limit(1)
      
      if (!tipos || tipos.length === 0) return 90 // fallback
      
      const tipoServicoId = tipos[0].id
      
      // Buscar preço específico da empresa para este tipo
      const { data: precoEmpresa } = await supabase
        .from('precos_tipos_empresa')
        .select('valor_unitario_decimal')
      ...
      if (precoEmpresa && precoEmpresa.length > 0) {
        return precoEmpresa[0].valor_unitario_decimal || 90
      }
      
      return 90 // fallback padrão
    } catch (error) {
      console.error('Erro ao buscar valor painel dos tipos:', error)
      return 90
    }
  }
  ...
  const carregarDados = async () => {
    ...
    try {
      ...
      // (Histórico por empresa/tipo é carregado por cartão da empresa)
    } catch (error) {
  ...
  // Removido: edição de preços por equipe (não faz parte da regra de negócio)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const dataToInsert = {
        ...
        valor_painel: (formData.tipo_remuneracao === 'painel' || formData.tipo_remuneracao === 'por_painel') ? formData.valor_painel : null
      }
  ...
  const handleEdit = async (empresa: Empresa) => {
    ...
    // Se for remuneração por painel, buscar valor dos tipos
    let valorPainel = 90
    if (empresa.tipo_remuneracao === 'painel' || empresa.tipo_remuneracao === 'por_painel') {
      valorPainel = await buscarValorPainelDosTipos(empresa.id)
    }
    ...
    setFormData({
      ...
      beneficiario_conta: empresa.beneficiario_conta || '',
      tipo_remuneracao: empresa.tipo_remuneracao as 'painel' | 'kwp' | 'por_painel' | 'por_kwp',
      valor_kwp: empresa.valor_kwp || 0,
      valor_painel: valorPainel
    })
    setShowForm(true)
  }
                      ...
                      <div className="p-6">
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex items-center text-sm text-gray-600">
                            ...
                            <span className="ml-1">{empresa.beneficiario_conta}</span>
                          </div>
                          ...
                          {(empresa.tipo_remuneracao === 'painel' || empresa.tipo_remuneracao === 'por_painel') && empresa.valor_painel && (
                            <div className="text-sm text-gray-600">
                              <span className="font-medium">Valor Painel:</span>
                              <span className="ml-1">R$ {empresa.valor_painel.toLocaleString('pt-BR')}</span>
                            </div>
                          )}
                              ...
                              {isExpanded && (
                                <div className="mt-3 space-y-2">
                                  {equipesEmpresa.map((equipe) => (
                                    <div key={equipe.id} className="p-3 bg-gray-50 rounded-lg border border-gray-200">
                                      <div className="flex items-center justify-between">
                                        <div className="flex items-center">
                                          <Users className="w-4 h-4 text-gray-400 mr-2" />
                                          <span className="text-sm text-gray-700">{equipe.nome}</span>
                                        </div>
                                        {/* Apenas nome da equipe; sem campos de preço */}
                                      </div>
                                    </div>
                                  ))}
                                </div>
                          ...
                          {/* Preços por Tipo movidos para o modal de edição da empresa */}
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <input
                            type="text"
                            value={formData.beneficiario_conta}
                            onChange={(e) => setFormData({...formData, beneficiario_conta: e.target.value})}
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {(formData.tipo_remuneracao === 'painel' || formData.tipo_remuneracao === 'por_painel') && (
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                              Valor por Painel (R$) *
                              <span className="text-xs text-gray-500 block">Valor obtido automaticamente dos "Preços por Tipo"</span>
                            </label>
                            <input
                              type="number"
                              step="0.01"
                              value={formData.valor_painel}
                      ...
                      {/* Edição de valores unitários ocorre abaixo em "Preços por Tipo" */}
                      {editingEmpresa && (
                        <EmpresaPrecosPorTipo 
                          empresaId={editingEmpresa.id} 
                          onValorPainelChange={(valor) => {
                            if (formData.tipo_remuneracao === 'painel' || formData.tipo_remuneracao === 'por_painel') {
                              setFormData(prev => ({ ...prev, valor_painel: valor }))
                            }
                          }}
                        />
                      )}
...
function EmpresaPrecosPorTipo({ empresaId, onValorPainelChange }: { empresaId: string, onValorPainelChange?: (valor: number) => void }) {
  const [loading, setLoading] = useState(false)
  const [rows, setRows] = useState<Array<{
    codigo: 'instalacao_painel' | 'instalacao_kwp'
    rotulo: string
    vigente?: number
    historico: Array<{ id: string; valor: number; de: string; ate?: string }>
    tipoId?: string
  }>>([
    { codigo: 'instalacao_painel', rotulo: 'Instalação (por painel)', historico: [] },
    { codigo: 'instalacao_kwp', rotulo: 'Instalação (por kWp)', historico: [] },
  ])
  ...
  useEffect(() => {
    const carregar = async () => {
      setLoading(true)
      try {
        const resTipos = await fetch('/api/tipos-servico?ativo=true', { cache: 'no-store' }).then(r => r.json())
        if (resTipos?.success) {
          const all = resTipos.data as any[]
          const next = await Promise.all(rows.map(async (r) => {
            const ts = all.find(t => t.codigo === r.codigo)
            if (!ts?.id) return r
            const resp = await fetch(`/api/empresas/${empresaId}/precos?tipo_servico_id=${ts.id}`).then(x => x.json()).catch(() => ({ success: false }))
            let vigente: number | undefined
            const historico: Array<{ id: string; valor: number; de: string; ate?: string }> = []
            if (resp?.success && Array.isArray(resp.data)) {
              resp.data.forEach((row: any, idx: number) => {
                const valor = row.valor_unitario_decimal as number
                const de = row.vigente_desde
                const ate = row.vigente_ate || undefined
                historico.push({ id: row.id, valor, de, ate })
                if (idx === 0) vigente = valor
              })
            }
            return { ...r, vigente, historico, tipoId: ts.id as string }
          }))
          setRows(next)
        }
      } finally {
  ...
  const salvar = async (rowIdx: number, valorStr: string) => {
    const row = rows[rowIdx]
    if (!row.tipoId || !valorStr) return
    const valor = Number(valorStr)
    const resp = await fetch(`/api/empresas/${empresaId}/precos`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ tipo_servico_id: row.tipoId, valor_unitario_decimal: valor, fechar_anterior: true })
    }).then(r => r.json())
    if (resp.success) {
      toast.success('Preço atualizado!')
      // refresh linha
      const novaLista = [...rows]
      novaLista[rowIdx] = { ...row, vigente: valor, historico: [{ id: resp.data?.id || String(Date.now()), valor, de: new Date().toISOString().slice(0,10) }, ...row.historico] }
      setRows(novaLista)
      
      // Se for alteração no valor de instalação por painel, notificar o componente pai
      if (row.codigo === 'instalacao_painel' && onValorPainelChange) {
        onValorPainelChange(valor)
      }
    } else {
      toast.error(resp.error || 'Erro ao salvar preço')
    }
  }

  return (
    <div className="border-t border-gray-200 pt-3 mt-3">
      <h4 className="text-sm font-semibold text-gray-900 mb-2">Preços por Tipo</h4>
      {loading ? (
        <div className="text-sm text-gray-500">Carregando preços...</div>
      ) : (
        <div className="space-y-2">
          {rows.map((r, idx) => (
            <div key={r.codigo} className="flex flex-col gap-2 p-2 bg-gray-50 rounded-lg">
              <div className="flex items-center justify-between">
                <div className="text-sm text-gray-700">{r.rotulo}</div>
                <div className="flex items-center gap-2">
                  <input
                    type="number"
                    step="0.01"
                    defaultValue={r.vigente}
                    placeholder="Valor"
                    className="w-32 px-2 py-1 border rounded"
                    onBlur={(e) => salvar(idx, e.target.value)}
                  />
                </div>
              </div>
                  ...
                  <div className="max-h-24 overflow-auto space-y-1 pr-1">
                    {r.historico.map((h) => (
                      <div key={h.id} className="flex justify-between">
                        <span>R$ {h.valor.toFixed(2)}</span>
                        <span>{new Date(h.de).toLocaleDateString()} {h.ate ? `→ ${new Date(h.ate).toLocaleDateString()}` : '(vigente)'}</span>
                      </div>
                    ))}
                  </div>
                </div>
