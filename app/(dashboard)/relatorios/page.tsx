...
export default function RelatoriosPage() {
  ...
  const [editandoLancamento, setEditandoLancamento] = useState<LancamentoComRelacoes | null>(null)
  ...
  const carregarRelatorio = async () => {
    ...
    try {
      ...
      if (filtros.equipes.length > 0) {
        query = query.in('equipe_id', filtros.equipes)
      }
      if (filtros.cliente) {
      ...
      const lancamentosComNomes = (data || []).map(lancamento => ({
        ...lancamento,
        equipe_nome: lancamento.equipes?.nome || 'N/A',
        empresa_nome: lancamento.equipes?.empresas?.nome || 'N/A',
        tipo_servico_nome: lancamento.tipos_servico?.nome || '-'
      }))
  ...
  const exportarCSV = () => {
    ...
    const csvContent = [
      ...
      ...lancamentos.map(item => [
        ...
        `"${item.empresa_nome}"`,
        `"${item.nome_cliente || ''}"`,
        `"${item.data_contrato ? format(new Date(item.data_contrato), 'dd/MM/yyyy', { locale: ptBR }) : ''}"`,
  ...
  const editarLancamento = (lancamento: LancamentoComRelacoes) => {
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {lancamentos.map((lancamento) => (
                      <tr key={lancamento.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {lancamento.data_contrato ? format(new Date(lancamento.data_contrato), 'dd/MM/yyyy', { locale: ptBR }) : '-'}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
